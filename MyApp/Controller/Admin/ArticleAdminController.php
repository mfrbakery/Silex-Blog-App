...
namespace MyApp\Controller\Admin{
  ...
  use MyApp\Model\Entity\Article;
  use MyApp\Form\ArticleForm;

  use MyApp\Model\Manager\ArticleManager;
  ...
  class ArticleAdminController implements ControllerProviderInterface {
    ...
    */
    public function post(Application $app) {
      ...
      $article->slug = \MyApp\Controller\Helper\String::slug($articleDatas["title"]) . "-" . md5(date("r"));
      ...
      $app["session"]->getFlashBag("success", "Article \"$article_[title]\" , $article_[_id] , saved !");
