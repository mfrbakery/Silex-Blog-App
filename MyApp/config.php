...
use MyApp\Model\Manager\SessionManager;
...
$app['autoloader']->add("MyApp",ROOT);
$app['autoloader']->add("Net",ROOT.'/Lib');
# providers
# twig
$app->register(new Silex\Provider\TwigServiceProvider(), array(
  "twig.path" => ROOT . "/MyApp/view",
  /**
...
$app['user_manager'] = $app->share(
  function($app) {
    return new \MyApp\Model\Manager\UserManager($app['config.mongo'], $app['config.database'],$app);

  }
  );
$app['user_provider']=$app->share(
  function($app){
    return new MyApp\Model\Provider\UserProvider($app['user_manager']);
  }
  );
# article manager
$app['article_manager'] = $app->share(
  function($app) {
    return new \MyApp\Model\Manager\ArticleManager($app['config.mongo'], $app['config.database']);
  }
  );
# comment manager
$app['comment_manager'] = $app->share(
  function(Silex\Application $app) {
    return new \MyApp\Model\Manager\CommentManager($app["config.mongo"], $app["config.database"]);
  }
  );
$app['spam_manager']=$app->share(
  function(Silex\Application $app){
    return new \MyApp\Model\Manager\SpamManager($app['config.mongo'],$app['config.database'],$_SERVER["HTTP_HOST"],$app['config.akismet_apikey']);
  }
);
/** @var $app['option_manager'] App\Model\Manager\OptionManager **/
$app['options']=$app->share( 
  function(Application $app){
    return new \MyApp\Model\Manager\OptionManager($app['config.mongo'],$app['config.database']);
  }
  );
# FILTERS
